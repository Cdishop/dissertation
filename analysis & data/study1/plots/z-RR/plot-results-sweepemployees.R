...
setwd("../../simulations/z-RR/sweep-people/sim-results")
...
ggplot(df %>% filter(
                     simulation %in% c('sim1a', 'sim1b', 'sim2a', 'sim2b')), aes(x = k, y = probability,
                                                                                 color = factor(numemployees))) +
                                                                                 #fill = factor(numemployees))) + 
  #geom_bar(stat = "identity", position = "dodge", alpha = 0.3) + 
  geom_line() + 
  facet_grid(situation ~ response) + 
  theme_classic(base_size = 15) + 
  labs(x = "Time Spent", y = "Probability", color = "Number of Agents")
...
ggplot(df %>% filter(
  simulation %in% c('sim3a', 'sim3b', 'sim4a', 'sim4b')), aes(x = k, y = probability,
                                                              color = factor(numemployees))) +
  #fill = factor(numemployees))) + 
  #geom_bar(stat = "identity", position = "dodge", alpha = 0.3) + 
  geom_line() + 
...
ggsave("manyfewsweeppeople.pdf", g, width = 8, height = 5.5)
ggsave("influxoutflowsweeppeople.pdf", g2, width = 8, height = 5.5)
