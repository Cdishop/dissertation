...
##########################################################################################################################
# sim4b: respond to outflux: no inertia


store_it_sim4b = DataFrame(
...
for sim in 1:sims
         ...
         for walk in 1:num_employees
             ...
             for step in 2:steps
                 ...
                 # make requests zero if it is negative
                  ...
                  # help is a a linear function of outgoing requests

                  if draw < 0
                      help[step] = draw
                  else
                      # no help if there are no outgoing requests
                      # i.e., if draw > 0
                      help[step] = 0
                  end
    ...
    # now I have (for a single simulation run) the number of time points employee x_i was in the top 20% when there were 2 employees
        store_it_sim4b[counter, :simulation] = sim
        store_it_sim4b[counter, :moment_citizen_counts] = moment_counts
...
for prob in prob_steps
    result = count(
        k == prob for k in store_it_sim4b.moment_citizen_counts) / length(store_it_sim4b.moment_citizen_counts)
            probability_list_2[[prob + 1]] .= result
end


sim4b_results = DataFrame(
...
CSV.write("sim-results/sim4b-linear.csv", sim4b_results)
